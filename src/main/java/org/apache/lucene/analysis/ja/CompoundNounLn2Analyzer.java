...
import org.apache.lucene.analysis.core.BothEndsFilterFactory;
...
public class CompoundNounLn2Analyzer extends Analyzer {

  private final UserDictionary userdic;
  ResourceLoader loader = new FilesystemResourceLoader(FileSystems.getDefault().getPath("."));

  public CompoundNounLn2Analyzer(){
    this(null);
  }

  public CompoundNounLn2Analyzer(String dic){
    if(dic != null){
      try {
  ...
  protected TokenStreamComponents createComponents(String fieldName) {
    ...
    BothEndsFilterFactory beff = new BothEndsFilterFactory(mapArg("tokenSeparator", "/", "reverse", "true"));
    try {
      sff.inform(loader);
    ...
    final TokenStream tokenStream = beff.create(
            lff.create(
                    prff3.create(
                            prff2.create(
                                    shff.create(
...
/*
<fieldType name="text_ja_ln2" class="solr.TextField" positionIncrementGap="100" autoGeneratePhraseQueries="true">
  <analyzer>
	...
	<filter class="com.rondhuit.nlp.lucene.ReverseTokensSequenceFilterFactory" delimiter="/"/>
  </analyzer>
</fieldType>
